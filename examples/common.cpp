...
std::string __get_tmpdir() {
        ...
        fprintf(stderr, "Using default temp dir: C:\\Temp\n");
        return "C:\\Temp";
    #else
        ...
        fprintf(stderr, "Using default temp dir: /tmp\n");
        return "/tmp";
    #endif
}
...
TryAgain:
    if (fname == "-") {
        ...
        fprintf(stderr, "%s: converting to wav...\n", fname.c_str());
        TRY_CONVERSION;
    }
    ...
    if (wav.channels != 1 && wav.channels != 2) {
        fprintf(stderr, "%s: audio file has %d channels\n", fname.c_str(), wav.channels);
        drwav_uninit(&wav);
        TRY_CONVERSION;
    }

    if (stereo && wav.channels != 2) {
        fprintf(stderr, "%s: audio file has %d channels (we want diarization)\n", fname.c_str(), wav.channels);
        drwav_uninit(&wav);
        TRY_CONVERSION;
    }

    if (wav.sampleRate != COMMON_SAMPLE_RATE) {
        fprintf(stderr, "%s: audio file has %d sample rate\n", fname.c_str(), wav.sampleRate);
        drwav_uninit(&wav);
        TRY_CONVERSION;
    }

    if (wav.bitsPerSample != 16) {
        fprintf(stderr, "%s: audio file has %d bits per sample\n", fname.c_str(), wav.bitsPerSample);
        drwav_uninit(&wav);
        TRY_CONVERSION;
    }
